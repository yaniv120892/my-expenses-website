...
Vendored from `yaniv120892/claude-config` (`rules/code.md`) so it loads for every
...
Language-agnostic. TypeScript specifics are in `typescript.md`. This repo has no Python,
so upstream's `python.md` is not vendored.
...
> references (`// ABC-123`); narrating _what_ rather than _why_.
